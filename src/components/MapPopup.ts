...
// Stub types for deleted tech-geo config and tech-activity service (arrays are empty, code never runs)
type StartupHub = {
  lat: number;
  lon: number;
  city: string;
...
type TechHubActivity = Record<string, never>;
...
export class MapPopup {
  ...
  private renderContent(data: PopupData): string {
    switch (data.type) {
      ...
      case "techHQCluster":
        return this.renderTechHQClusterPopup(
          data.data as { items: TechHQ[]; city: string; country: string },
        );
  ...
  private renderProtestClusterPopup(data: ProtestClusterData): string {
    ...
    const totalFatalities = data.items.reduce(
    ...
    const moreCount =
      data.items.length > 10
        ? `<li class="cluster-more">+${data.items.length - 10} more events</li>`
        : "";
    const headerClass =
      highSeverity.length > 0 ? "high" : riots.length > 0 ? "medium" : "low";
  ...
  private renderDatacenterClusterPopup(data: DatacenterClusterData): string {
    ...
    const totalPower = data.items.reduce(
      (sum, dc) => sum + (dc.powerMW || 0),
      0,
    );
    const existingCount = data.items.filter(
      (dc) => dc.status === "existing",
    ).length;
    const plannedCount = data.items.filter(
      (dc) => dc.status === "planned",
    ).length;
      ...
      <div class="popup-body">
        ...
        <div class="popup-stats">
          ...
          <div class="popup-stat">
            ...
            <span class="stat-value">${escapeHtml(String(hq.marketCap))}</span>
          </div>
          `
              : ""
          }
  ...
  private renderTechHQClusterPopup(data: {
    items: TechHQ[];
    city: string;
    country: string;
  }): string {
  ...
  private renderTechEventClusterPopup(data: {
    items: TechEventPopupData[];
    location: string;
    country: string;
  }): string {
