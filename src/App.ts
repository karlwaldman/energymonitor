...
export class App {
  ...
  constructor(containerId: string) {
    ...
    const storedVariant = localStorage.getItem("energymonitor-variant");
    ...
    if (storedVariant !== currentVariant) {
      ...
      localStorage.setItem("energymonitor-variant", currentVariant);
  ...
  private setupCountryIntel(): void {
    ...
    this.map.onCountryClicked(async (lat, lon) => {
      ...
      const stockPromise = fetch(
        `/api/stock-index?code=${encodeURIComponent(geo.code)}`,
      )
        .then((r) => r.json())
      ...
      // Prediction markets removed (polymarket service deleted)
      this.countryIntelModal!.updateMarkets([]);
      ...
      try {
        ...
        try {
          ...
          /* server unreachable — will fall through to browser fallback */
        }

        if (data && data.brief && !data.skipped) {
          this.countryIntelModal!.updateBrief({
            ...data,
            code: geo.code,
          } as Parameters<typeof this.countryIntelModal.updateBrief>[0]);
        } else {
          ...
          if (fallbackBrief) {
            ...
            if (signals.militaryFlights > 0)
              lines.push(
                `${signals.militaryFlights} military aircraft tracked`,
              );
    ...
    if (this.intelligenceCache.protests?.events) {
      protests = this.intelligenceCache.protests.events.filter(
        (e) =>
          ...
          (hasBbox && this.isInCountry(e.lat, e.lon, code)),
      ).length;
    }
    ...
    if (this.intelligenceCache.outages) {
      outages = this.intelligenceCache.outages.filter(
        (o) =>
          ...
          (hasBbox && this.isInCountry(o.lat, o.lon, code)),
      ).length;
    }
  ...
  private openCountryStory(code: string, name: string): void {
    ...
    const postures: TheaterPostureSummary[] = [];
      ...
      <div class="header">
        <div class="header-left">
          ${
            SITE_VARIANT === "energy"
              ? `
          <span class="energy-brand">
            <span class="energy-brand-icon">&#9889;</span>
            <span class="logo">ENERGY MONITOR</span>
          </span>
          <a href="https://oilpriceapi.com?ref=energymonitor" target="_blank" rel="noopener" class="credit-link">
            <span class="credit-text">by OilPriceAPI</span>
          </a>
          `
              : `
          <div class="variant-switcher">
            <a href="${this.isDesktopApp ? "#" : SITE_VARIANT === "tech" ? "https://energy-monitor.app" : "#"}"
            ...
            <a href="${this.isDesktopApp ? "#" : SITE_VARIANT === "tech" ? "#" : "https://tech.energy-monitor.app"}"
               class="variant-option ${SITE_VARIANT === "tech" ? "active" : ""}"
               data-variant="tech"
               ${!this.isDesktopApp && SITE_VARIANT !== "tech" ? 'target="_blank" rel="noopener"' : ""}
          ...
          <a href="https://github.com/karlwaldman/energymonitor" target="_blank" rel="noopener" class="github-link" title="View on GitHub">
      ...
      <div class="main-content">
        <div class="map-section" id="mapSection">
          <div class="panel-header">
            ...
            ${SITE_VARIANT === "energy" ? '<a class="api-cta" href="https://oilpriceapi.com" target="_blank" rel="noopener">Get via API ↗</a>' : ""}
  ...
  private createPanels(): void {
    ...
    // OPA Prices panel (energy variant - live commodity prices from OilPriceAPI)
    const opaPricesPanel = new OPAPricesPanel();
    this.panels["opa-prices"] = opaPricesPanel;
        ...
        .forEach((link) => {
          link.addEventListener("click", (e) => {
            ...
            if (variant && variant !== SITE_VARIANT) {
              e.preventDefault();
              localStorage.setItem("energymonitor-variant", variant);
  ...
  private async loadNews(): Promise<void> {
    ...
    // Run correlation analysis (fire-and-forget via Web Worker)
  ...
  private async loadNatural(): Promise<void> {
    // Load NASA EONET natural events (storms, fires, volcanoes, etc.)
    try {
      const events = await fetchNaturalEvents(30);
      this.map?.setNaturalEvents(events);
      ...
      this.map?.setLayerReady("natural", false);
    }
  }
  ...
  private async loadIntelligenceSignals(): Promise<void> {
    ...
    console.log("[Intelligence] All signals loaded");
  }
