...
let latestTankers: AisPositionData[] = [];
...
function parseSnapshot(data: unknown): {
  ...
  return {
    ...
    candidateReports: Array.isArray(raw.candidateReports)
      ...
      : [],
    tankerReports: Array.isArray(raw.tankerReports) ? raw.tankerReports : [],
  };
}
...
async function fetchSnapshotPayload(
  ...
  const query = `?candidates=${includeCandidates ? "true" : "false"}&tankers=true`;
...
async function pollSnapshot(force = false): Promise<void> {
  ...
  try {
    ...
    latestTankers = snapshot.tankerReports.map((r) => ({
      mmsi: r.mmsi,
      name: r.name || "",
      lat: r.lat,
      lon: r.lon,
...
export async function fetchAisSignals(): Promise<{
  disruptions: AisDisruptionEvent[];
  density: AisDensityZone[];
  tankers: AisPositionData[];
}> {
  if (!aisConfigured) {
    return { disruptions: [], density: [], tankers: [] };
  }
