...
export function generateStoryDeepLink(
  ...
  return `https://energy-monitor.app/api/story?${params.toString()}`;
}
...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via EnergyMonitor - Real-time energy intelligence`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - EnergyMonitor`)}`,
