...
export default async function handler(req) {
  ...
  const includeTankers = requestUrl.searchParams.get("tankers") === "true";
  const cacheKey = `ais-snapshot:${CACHE_VERSION}:${includeCandidates ? "full" : "lite"}:${includeTankers ? "tankers" : "notankers"}`;
  ...
  try {
    ...
    if (!requestPromise) {
      requestPromise = (async () => {
        const upstreamUrl = `${relayBaseUrl}/ais/snapshot?candidates=${includeCandidates ? "true" : "false"}&tankers=${includeTankers ? "true" : "false"}`;
